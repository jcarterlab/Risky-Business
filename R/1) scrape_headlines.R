...
clean_text <- function(text) {
  whitespace <- str_trim(text)
  clean <- str_replace_all(whitespace, pattern, "")
  return(clean)
}
...
get_raw_data <- function(link) {
  text <- read_link(link)
  clean_text <- clean_text(text)
  final_text <- drop_short_headlines(clean_text)
  tibble <- final_text %>%
    as_tibble() %>%
    mutate(Site = pull_info(link, "link"),
...
get_headlines <- function(links) {
  table <- lapply(links, get_headline_data)
  headlines <- distinct(bind_rows(list(table)))
  headlines$Headline <- replace_na(headlines$Headline, "")
  return(headlines)
}

# reads in the relevant links data frame. 
read_links_df <- function(region) {
  links <- read_csv(paste0("C:\\Users\\HUAWEI\\Desktop\\Projects\\Risky-Business\\data\\Links\\",
                           region,
                           "_links.csv"))
  return(links)
}
